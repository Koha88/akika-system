<template>
  ...
  <validation-provider name="TotalAmount">
    <b-form-group :label="$t('TotalAmount')">
      <b-input-group :append="currentUser.currency">
        <b-form-input
          :value="clientReport ? formatNumber(clientReport.total_amount, 2) : ''"
          readonly
        ></b-form-input>
      </b-input-group>
    </b-form-group>
...
<b-col lg="4" md="4" sm="12">
  <validation-provider name="ClientDue">
    <b-form-group label="Долг клиента">
      <b-input-group :append="currentUser.currency">
        <b-form-input
          :value="clientReport ? (clientReport.due > 0 ? '-' + formatNumber(clientReport.due, 2) : '0.00') : ''"
          :class="{
            'border-danger text-danger font-weight-bold': clientReport && clientReport.due > 0,
            'border-success text-success font-weight-bold': clientReport && clientReport.due === 0
          }"
          readonly
        ></b-form-input>
      </b-input-group>
...
<b-col lg="4" md="4" sm="12">
  <validation-provider name="TotalSales">
    <b-form-group label="Количество покупок">
      <b-input-group>
        <b-form-input
          :value="clientReport ? clientReport.total_sales : ''"
          :class="{
            'border-success text-success font-weight-bold': clientReport && clientReport.total_sales > 0
          }"
          readonly
        ></b-form-input>
      </b-input-group>
...
<b-row v-if="clientDebts.length" class="mt-3">
  <b-col cols="12">
    <h5>Подробности долга клиента</h5>
    <b-table :items="clientDebts" :fields="[
      { key: 'date', label: 'Дата' },
      { key: 'Ref', label: '№' },
      { key: 'GrandTotal', label: 'Сумма' },
      { key: 'paid_amount', label: 'Оплачено' },
      { key: 'due', label: 'Остаток' },
      { key: 'days_overdue', label: 'Дней с продажи' }
    ]" bordered small></b-table>
  </b-col>
</b-row>
...
<b-col lg="4" md="4" sm="12">
  ...
  >
    <b-form-group :label="$t('TotalAmount')">
      ...
      <b-form-invalid-feedback id="TotalAmount-feedback">
...
export default {
  ...
  Submit_Payment() {
    ...
    if (this.sale.client_id) {
      axios
        .get('/report/client/' + this.sale.client_id)
        .then((response) => {
          this.clientReport = response.data.report;
          this.loadClientDebtDetails();
        })
        .catch((error) => {
  ...
  loadClientDebtDetails() {
  if (this.sale.client_id) {
    axios
      .get('/report/client_debts/' + this.sale.client_id)
      .then((response) => {
        this.clientDebts = response.data.debts;
      })
      .catch((error) => {
