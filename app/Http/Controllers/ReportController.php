...
public function Client_Debt_Details(Request $request, $id)
{
    $this->authorizeForUser($request->user('api'), 'Reports_customers', Client::class);
    ...
    $debts = DB::table('sales')
        ...
        ->select([
            'Ref', 'date', 'GrandTotal', 'paid_amount',
            DB::raw('(GrandTotal - paid_amount) as due'),
            DB::raw('DATEDIFF(NOW(), date) as days_overdue')
        ])
        ->orderBy('date', 'desc')
        ->get();
